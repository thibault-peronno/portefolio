...
<section class="section">
    ...
    <form action="" id="project-form" method="post" enctype="multipart/form-data">
        <div class="mb-5">
            ...
            <input type="text" name="label" id="label" value="<?= isset($language) ? $language->get_label() : " " ?>" class="rounded bg-white h-12 w-full p-2" />
        </div>
        <span class="sm:flex sm:flex-row sm:gap-5 sm:flex-nowrap">
            <div class="mb-5 sm:w-6/12">
                ...
                <select name="type" id="type" class="block rounded bg-white h-12 w-full p-2">
                    <option value="<?= isset($language) ? $language->get_type() : "" ?>"><?= isset($language) ? $language->get_type() : "Type" ?></option>
            ...
            <?php if (isset($language)) : ?>
                <div id="updateImageTextDiv">
                    <img class="p-2.5 bg-white w-14" src="<?= "/assets/images/languages/" . $language->get_picture() ?>" alt="Image du projet" class="w-14">
                    <p class="font-bold text-white cursor-pointer" id="updateImageText">Modifier l'image</p>
                    <input type="hidden" name="picture" value="<?= $language->get_picture() ?>">
                </div>
            <?php endif ?>
